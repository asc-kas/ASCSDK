...
-   ### 非必接功能(Non-required function)
    ...
    -   #### OBB挂载
        ...
        -   public void getDownloadUrl(final String partID)
    ...
    -   #### 互推功能(Elect function)
        ...
        -   public void showElect(String type)
            -   展示推荐(Shows the recommended)
        -   type 是指弹出的类型
            -   start 游戏未通关(The game starts to pop up automatically)
            -   finsh 游戏通关(Play through completion)
            -   click 点击icon(Game more game active click)
...
-   ### 道具统计(Props statistics)
    ...
    -   <font color=red >用户基本事件统计(必接 Must be)</font>
        -   public void setUserInfo(final String level,final String buff,final String coin)
            -   level 用户当前最高关卡(User current highest level)
            -   buff 用户当前剩余道具数量(The current number of items left by the user)
            -   coin 用户当前剩余金币数量(The current number of gold COINS left by the user)
